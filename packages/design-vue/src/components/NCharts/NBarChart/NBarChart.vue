<template>
  <div class="n-bar-chart">
    <div v-if="isEmpty" class="n-chart-empty">
      ...
      >
        <path
          d="M3 3v18h18M7 11v6M12 7v10M17 14v3"
...
import type { NBarChartProps } from '../types'

const props = withDefaults(defineProps<NBarChartProps>(), {
  height: 300,
  hideLegend: false,
  hideTooltip: false,
  yGridLine: true,
...
const yAxisKeys = computed(() => props.series.map((s) => s.key))

const chartAriaLabel = computed(() => buildChartAriaLabel('막대 차트', props))
...
const internalOrientation = computed(() =>
  props.orientation === 'horizontal' ? Orientation.Horizontal : Orientation.Vertical,
)
...
/* Axis labels */
.n-bar-chart :deep(.tick text),
.n-bar-chart :deep(.axis text) {
...
/* Grid lines */
.n-bar-chart :deep(.grid line),
.n-bar-chart :deep(.grid path) {
  stroke: var(--n-color-neutral-200, #e5e5e5);
}

/* Tooltip 껍데기 (내부 마크업은 NChartTooltip) */
.n-bar-chart :deep([class*='tooltip']:not([class*='n-chart-tooltip'])),
.n-bar-chart :deep([class*='Tooltip']:not([class*='n-chart-tooltip'])) {
...
.n-bar-chart :deep([class*='bullet-legend']) {
...
/* Hover: 마우스 올린 막대 색을 진하게 — 시각 피드백.
   unovis stacked/grouped-bar 는 막대를 <path>(label: bar)로 렌더 → rect 아님. path[class*=bar] 로 타겟. */
.n-bar-chart :deep(path[class*='bar']) {
  transition: filter 0.12s ease;
}
.n-bar-chart :deep(path[class*='bar']:hover) {
  filter: brightness(0.82);
}

/* loading — 축·라벨 없이 플롯 자리만 채우는 스켈레톤 */
.n-bar-chart .n-chart-loading {
  width: 100%;
