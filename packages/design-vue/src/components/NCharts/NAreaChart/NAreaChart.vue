<template>
  <div class="n-area-chart">
    ...
    <!-- role=status + aria-busy — 스켈레톤은 «시각적 자리표시»라 낭독기에는 아무것도
         ...
         (NTableSkeleton / NDetailSkeleton 과 동일 규약) -->
    <div
      v-else-if="loading"
...
import type { NAreaChartProps } from '../types'

const props = withDefaults(defineProps<NAreaChartProps>(), {
  height: 300,
  hideLegend: false,
  hideTooltip: false,
  yGridLine: true,
...
const chartAriaLabel = computed(() => buildChartAriaLabel('영역 차트', props))
...
// Tooltip: 공용 헬퍼 + NChartTooltip 으로 통일 (기본 템플릿의 divider/capitalize 제거)
...
/* Axis labels */
.n-area-chart :deep(.tick text),
.n-area-chart :deep(.axis text) {
...
/* Grid lines */
.n-area-chart :deep(.grid line),
.n-area-chart :deep(.grid path) {
  stroke: var(--n-color-neutral-200, #e5e5e5);
}

/* Tooltip 껍데기 (내부 마크업은 NChartTooltip) — 흰 서피스 + 표준 그림자 */
.n-area-chart :deep([class*='tooltip']:not([class*='n-chart-tooltip'])),
.n-area-chart :deep([class*='Tooltip']:not([class*='n-chart-tooltip'])) {
...
/* Legend */
.n-area-chart :deep([class*='legend']),
.n-area-chart :deep([class*='Legend']) {
...
.n-area-chart :deep([class*='bullet-legend']) {
...
/* loading — 축·라벨 없이 플롯 자리만 채우는 스켈레톤 (NKpi 와 동일 톤/타이밍) */
.n-area-chart .n-chart-loading {
  width: 100%;
...
/* 시각적으로 감추되 접근성 트리에는 남긴다 (styles/base.css 의 .n-sr-only 와 동일 규칙 —
   charts 서브엔트리는 base.css 를 import 하지 않으므로 컴포넌트 로컬로 재정의한다) */
.n-area-chart .n-chart-sr-only {
  position: absolute;
  width: 1px;
  height: 1px;
  padding: 0;
