...
- **사업자정보 · 고객센터 · 법정 문안** — `app/content/{business,support,legal}.ts`. 미확정 값은 `P9_4_PENDING`(화면 «(확정 전)»). ⛔ **main 머지 게이트**: `grep -rln P9_4_PENDING apps/client/app | grep -v content/pending.ts` 가 빈 결과. 환불정책에 «QR 발급 후 수수료 · 공제 · 청약철회 제한» 을 넣으면 `legal.test.ts` 가 막는다(A5).
