...
| 제약                                                                                                  | 프로세스 대응                                                                                               |
| ----------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| **admin/client 테스트는 얇다** (INF-1 로 러너는 붙었다 — client 647건(2026-09-23 W1-3 QA ⑥ 2회차 반영), admin 0건) | 회귀 테스트를 **우선 시도**하고, 대상이 순수 로직이 아니면 검증 증거(커맨드 출력·Orca 스크린샷)로 대체 + plan 에 사유 명시 |
| **검증 3층이 갖춰졌다** — 로컬(INF-1) · PR/main CI(INF-2) · Dockerfile(INF-3)                          | 세 곳이 **같은 스크립트·같은 기준선**(`typecheck-gate.sh`, admin 0 / client 4 — 2026-09-23 W1-2 에서 7 → 4)을 쓴다. 다만 게이트는 «타입» 만 본다 — 동작 검증은 `nomacomfe-prod-push-check` 의 UI 수동 확인 몫 |
