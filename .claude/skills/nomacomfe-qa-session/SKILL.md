...
검증 자산 검사 필수: 테스트가 있으면 assertion 이 실제로 있는가 / 대상 모듈을 mock 해
  ...
  "테스트 인프라 부재" 사유가 명시돼 있고 대체 증거(커맨드 출력·스크린샷)가 붙었는가.
...
- **walk ≠ 회귀 커버리지**: 세션에서 브라우저로 확인한 것은 회귀 자산이 아니다. 편입 판정된 시나리오는 테스트 코드로 승격해야 자산이 된다. **admin/client 는 아직 러너가 없으므로**(→ `nomacomfe-write-plan` 테스트 환경 제약) 편입 대상을 **목록으로 남겨** Phase 3 테스트 부트스트랩 트랙의 입력으로 삼는다.
